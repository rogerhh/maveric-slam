...
def draw_matches_with_epilines(img1, img2, pts1, pts2, F):
    ...
    img1_epilines = draw_lines(img1_color, lines1, pts1)

    # Draw the epipolar lines on the first image
    lines2 = cv2.computeCorrespondEpilines(pts1.reshape(-1, 1, 2), 1, F)
    lines2 = lines2.reshape(-1, 3)
    img2_epilines = draw_lines(img2_color, lines2, pts2)

    # Concatenate the images and show them side by side
    combined = np.hstack((img1_epilines, img2_epilines))
    cv2.imwrite('epilines.png', combined)

def draw_lines(img, lines, pts):
    """Draw lines on the image for each corresponding point."""
    r, c = img.shape[:2]
...
if __name__ == '__main__':
  ...
  parser.add_argument("--outfile", type=str, default=None, 
      help="name of the file to write the transform to")
  ...
  # Example points (these should be your corresponding points)
  pts_img1 = np.array(match0, dtype='float32')
  pts_img2 = np.array(match1, dtype='float32')
  
  # Camera calibration matrix (example values, should be replaced with your calibration matrix)
  K = np.array([[718.856, 0, 607.1928],
                [0, 718.856, 185.2157],
                [0, 0, 1]], dtype='float64')

  # Compute the Essential matrix
  E, mask = cv2.findEssentialMat(pts_img1, pts_img2, K)

  print(E)
  print(mask)
  
  # Recover the pose from the Essential matrix
  w1, R, t, w2 = cv2.recoverPose(E, pts_img1, pts_img2, K, mask)
  ...
  # If needed, you can compose the transformation matrix as:
  T = np.hstack((R, t))
  print("\nTransformation matrix [R|t]:")
  print(T)

  np.save(opt.outfile, T)

  draw_matches_with_epilines(img0, img1, pts_img1[:10].astype(int), pts_img2[:10].astype(int), E)
